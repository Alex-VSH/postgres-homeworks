...
WHERE p.discontinued = 1 AND o.product_id = p.product_id);
DELETE FROM products WHERE (discontinued = 1)
-- с этим заданием мне помогли, насколько я понял чтобы не потерялась связь мы из order_details удаляем заказы где есть
-- товары, снятые с продажи, хотелось бы после сдачи домашки обратной связи по этой части задания
